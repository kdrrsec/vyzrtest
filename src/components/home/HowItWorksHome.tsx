  ...
  return (
    <section className={isPage ? "pt-4 pb-12" : "py-20"}>
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-400">{t("homeLead")}</p>

        <div className="mt-12 grid gap-8 md:grid-cols-3">
          {steps.map((s) => (
            <div key={s.key} className="border-t border-white/25 pt-6">
              <p className="text-xs font-semibold uppercase leading-snug tracking-[0.14em] text-accent [text-shadow:0_0_20px_rgba(255,45,45,0.35)] sm:text-[13px]">
                {s.title}
              </p>
              <p className="mt-4 whitespace-pre-line text-sm font-normal leading-relaxed text-zinc-300">
                {s.body}
              </p>
            </div>
          ))}
        ...
        <div
          id="delivery-options"
          className="mt-16 scroll-mt-28 border-t border-white/20 pt-14 md:mt-20 md:pt-16"
        >
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 md:gap-8">
            {deliveryOptions.map((option) => (
              <article
                key={option.title}
                className="flex flex-col rounded-2xl border border-white/[0.12] bg-white/[0.02] p-6 md:p-8"
              >
                <div>
                ...
                <ul className="mt-6 space-y-2.5 border-t border-white/10 pt-6">
                  {option.highlights.map((item) => (
                    <li key={item} className="flex gap-2.5 text-sm leading-snug text-zinc-300">
          ...
          <p
            className="mt-8 rounded-2xl border border-accent/25 bg-accent/[0.06] px-5 py-4 text-center text-sm font-medium leading-relaxed text-zinc-200 shadow-[0_0_24px_-8px_rgba(255,45,45,0.25)] md:mt-10 md:px-6 md:py-5"
            role="note"
          >
        ...
        <p
          className={`mx-auto max-w-2xl text-center text-sm leading-relaxed text-zinc-300 ${
