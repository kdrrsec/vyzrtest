    ...
    <>
      ...
      <header className="sticky top-0 z-50 overflow-visible border-b border-black/10 bg-white/90 backdrop-blur-md dark:border-white/10 dark:bg-black/90">
        <div className="relative mx-auto flex h-[50px] max-h-[50px] min-h-[50px] max-w-7xl items-center justify-between gap-3 overflow-visible px-4 md:px-6">
          ...
          {/* Mobile/tablet only: absolutely positioned with equal left/right insets (112px,
              just past the ~110px-wide icons group - the wider of the two sides) so it's
              centered against the real screen middle, not the leftover gap next to whichever
              side is narrower (centering against that gap directly shifted it ~27px off
              true-center, since the icons side sits closer to center than the hamburger side).
              `inset-y-0` (not `self-stretch` in a height-capped flex/grid row) is what makes
              vertical centering work when the logo is taller than the 50px header: it gives
              the positioned box a definite 50px height that `items-center` can center an
              oversized child within, overflowing symmetrically above and below. A grid/flex
              item alignment property can't do that once the row itself is height-capped
              below the item's own content size - it just gets pinned to the row's start
              edge and overflows entirely downward instead. */}
          <Link
            href={PATHS.home}
            className="absolute inset-y-0 left-[112px] right-[112px] z-10 flex items-center justify-center overflow-visible lg:hidden"
            aria-label={tNav("home")}
          >
...
function HeaderLogo({ src, mobile }: { src: string; mobile?: boolean }) {
  ...
  return (
    ...
    <img
      ...
      className={
        mobile
          ? "h-14 w-auto max-w-full shrink-0 object-contain brightness-0 dark:brightness-100 min-[375px]:h-20"
